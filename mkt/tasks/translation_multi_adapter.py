...
@register_task("translation_multi_adapter")
class TranslationMultiAdapterTask(LegacyFairseqTask):
    """
    ...
    def add_args(parser):
        ...
        # custom
        parser.add_argument('--vocab-adapter', action='store_true',
                            help='whether apply the strategy of vocabulary adapter')
        parser.add_argument('--freeze-all', action='store_true',
                            help='freeze all parameters at training step')
        parser.add_argument('--base-embed-dim', default=0, type=int, 
                             help='addintional embedding dims')
        parser.add_argument('--krail-model', default='', type=str, 
                            help='load the external model with path')
        parser.add_argument('--new-dict-path', default='', type=str, 
                            help='the dict of vex dictionary')
        parser.add_argument('--krail-dict-path', default='', type=str, 
                            help='the dict of krail dictionary')
        parser.add_argument('--no-adapter-relu', action='store_true',
                            help='whether apply the ReLU strategy in adapter')
        parser.add_argument('--krail-module', default='none', type=str, 
                            help='whether apply the strategy of adapter in all modules')
        parser.add_argument('--adapter-module', default='none', type=str, 
                            help='whether apply the strategy of adapter in serial or parallel or none')
    ...
    def check_dicts(self, dicts, source_langs, target_langs):
        ...
        for src_lang in source_langs:
            ...
            "TranslationMultiAdapterTask only supports one shared dictionary across all source languages"
        for tgt_lang in target_langs:
            assert (
                tgt_dict == dicts[tgt_lang]
            ), "Diffrent dictionary are specified for different target languages; "
            "TranslationMultiAdapterTask only supports one shared dictionary across all target languages"
