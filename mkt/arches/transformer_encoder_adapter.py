...
from .transformer_adapter_layer import TransformerEncoderLayerAdapter
from .transformer_base_layer import TransformerEncoderLayerBase
from ..misc.load_krail import load_checkpoint_to_cpu

# rewrite name for backward compatibility in `make_generation_fast_`
def module_name_fordropout(module_name: str) -> str:
    if module_name == "TransformerEncoderBaseAdapter":
        return "TransformerEncoderAdapter"
    else:
        return module_name


class TransformerEncoderBaseAdapter(FairseqEncoder):
    """
    ...
    def __init__(self, cfg, dictionary, embed_tokens, base_embed_tokens, return_fc=False):
        self.cfg = cfg
        ...
        if cfg.layernorm_embedding:
            ...
            if cfg.freeze_all:
                self.layernorm_embedding.bias.requires_grad = False
                self.layernorm_embedding.weight.requires_grad = False
        else:
        ...
        if cfg.adapter_module == 'serial_single':
            self.layers.extend(
                [self.build_encoder_nonfinal_layer(cfg) for i in range(cfg.encoder.layers-1)]
            )
            self.layers.append(self.build_encoder_final_layer(cfg))
        else:
            self.layers.extend(
        ...
        if cfg.krail_model:
            krail_state_dict = load_checkpoint_to_cpu(cfg.krail_model)

    def build_encoder_layer(self, cfg):
        if cfg.adapter_module != 'none':
            layer = TransformerEncoderLayerAdapter(
    ...
    def build_encoder_nonfinal_layer(self, cfg):
            # layer = transformer_layer.TransformerEncoderLayerBase(
            #     cfg, return_fc=self.return_fc
            # ) 
    ...
    def build_encoder_final_layer(self, cfg):
            if cfg.adapter_module != 'none':
                layer = TransformerEncoderLayerAdapter(
                    cfg, return_fc=self.return_fc
                )
    ...
    def forward_embedding(
        ...
        if self.base_embed_tokens is not None:
            split_dim = self.base_embed_tokens.weight.size(0)
            
            self.embed_tokens.weight.requires_grad = False
           
            self.embed_tokens.weight.data[:split_dim] = self.base_embed_tokens.weight.data


            self.embed_tokens.weight.requires_grad = True
        elif self.base_embed_tokens is None and self.cfg.freeze_all:
            self.embed_tokens.weight.requires_grad = False
        else:
            self.embed_tokens.weight.requires_grad = False
...
class TransformerEncoderAdapter(TransformerEncoderBaseAdapter):
